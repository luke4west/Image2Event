...
class NormGradient(torch.nn.Module):
    def __init__(self, ignore_border=False, return_norm=True):
        """
    ...
    def forward(self, x):
        ...
        gradient_x = torch.nn.functional.conv2d(x, self.filter_matrix_x.to(x.device), padding=0, stride=1)
        gradient_y = torch.nn.functional.conv2d(x, self.filter_matrix_y.to(x.device), padding=0, stride=1)
